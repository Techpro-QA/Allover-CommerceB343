...
public class US_02 {
    ...
    public void cannotRegisterWithExistingData (String username, String email, String password) {
        ...
        homePage.registerUserName.sendKeys(username);
        homePage.registerEmail.sendKeys(email);
        homePage.registerPassword.sendKeys(password);

        if (!homePage.registerPrivacyPolicyCheckBox.isSelected()) {
            homePage.registerPrivacyPolicyCheckBox.click();
        }
